...
function Shop() {
  ...
  return (
    <div className='min-h-screen bg-[#fff5f2]'>
      ...
      {/* BACK */}
      <button
        className='fixed top-4 left-4 z-20 flex items-center gap-2 bg-white/90 hover:bg-white text-[#E23744] px-4 py-2 rounded-full shadow-lg transition'
        onClick={() => navigate("/")}>
        <FaArrowLeft />
        <span>Back</span>
      </button>
      ...
      <div className='max-w-7xl mx-auto px-4 sm:px-6 py-12'>
        ...
        <h2 className='flex items-center justify-center gap-3 text-2xl sm:text-3xl font-bold mb-10 text-gray-800'>
          <FaUtensils className='text-[#E23744]' />
          Our Menu
        </h2>
        ...
        {items.length > 0 ? (
          <div className='flex flex-wrap justify-center gap-6 sm:gap-8'>
            {items.map((item, i) => (
              <div key={i} className='bg-white rounded-2xl shadow-sm hover:shadow-xl transition'>
                <FoodCard data={item} />
              </div>
            ))}
          </div>
